...
const SignUp = ({ onForgotPasswordClick }) => {
  ...
  const [visibleErrormobile, setVisibleErrormobile] = useState("");
  ...
  useEffect(() => {
    if (errors.mobile) {
      setVisibleErrormobile(errors.mobile);
    } else {
      const timer = setTimeout(() => setVisibleErrormobile(""), 500);
      return () => clearTimeout(timer);
    }
  }, [errors.mobile]);
  useEffect(() => {
    if (errors.email) {
      setVisibleErrorEmail(errors.email);
    } else {
      const timer = setTimeout(() => setVisibleErrorEmail(""), 500);
      return () => clearTimeout(timer);
    }
  }, [errors.email]);
  ...
  const validate = (fieldValues = formData, agreedValue = agreed) => {
    const tempErrors = { mobile: "", password: "", agreed: "" };
    const mobileRegex = /^[a-zA-Z0-9]{3,20}$/;

    if ("mobile" in fieldValues) {
      if (!fieldValues.mobile) tempErrors.mobile = "mobile is required.";
      else if (!mobileRegex.test(fieldValues.mobile))
        tempErrors.mobile =
          "mobile must be 3-20 characters, letters and numbers only.";
    }
  ...
  return (
    <div className="w-full flex flex-col  gap-0 ">
      <div className=" w-max flex flex-row  items-center gap-3  pb-5 ">
        <div>
        ...
        <form className="flex flex-col gap-4 w-full" onSubmit={handleSubmit}>
          ...
          <div className="w-full h-16">
            ...
            <div
              className={`px-2 text-red-500 text-sm mt-1 transition-opacity duration-500 ease-in-out overflow-hidden min-h-5
                  ${
                    errors.mobile ? "opacity-100 max-h-20" : "opacity-0 max-h-0"
                  }`}
            >
              {visibleErrormobile || " "}
            </div>
          ...
          <div className="w-full h-16">
            ...
            <div
              className={`px-2 text-red-500 text-sm mt-1 transition-opacity duration-500 ease-in-out overflow-hidden min-h-5
                  ${
                    errors.email ? "opacity-100 max-h-20" : "opacity-0 max-h-0"
                  }`}
            >
              {visibleErrorEmail || " "}
            </div>
